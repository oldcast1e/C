...
int main(){
    ...
    TAXI rstst;
    for(int i=0;i<N;i++){
        if(strcmp(system[i].start,startarr)==0){
            // printf("check");
            for(int j=0;j<N;j++){
                if(i!=j){
                    if(strcmp(system[i].target,endarr)==0){
                        // printf(">[%d]%s ==  [%d]%s\n",i,system[i].target,j,endarr);
                        if(system[i].tot > max){
                            // printf(">%d\n",system[i].tot);
                            max = system[i].tot;
                            rstst = system[i];
    ...
    printf("%.2lf %d %c %d",rstst.distance,rstst.t_time,rstst.late_night,rstst.tot);
