...
int main(){
    ...
    if(N%3==1 || N%3==2){
        ...
        //최대값
        cnt = 0;
        for(int i=0;i<N/3;i+=3){//ex)5이면 1번반복(1,2,3)/8이면 2번(1,2,3/4,5,6)반복
        ...
        /////////체크
        printf("\n");
        for(int i=0;i<N;i++){//n-1번째 부터 0번째 배열 요소를 
        if(max[i]>0)
            printf(" %d",max[i]);//출력
        }
        //////////
        ...
        for(int i=0;i<N/3+1;i+=3){//ex)5이면 1번반복(1,2,3)/8이면 2번(1,2,3/4,5,6)반복
            if(num[i]<num[i+1] && num[i]<num[i+2] ){
                min[cnt] = num[i];
                printf("<%d>",cnt);
                cnt++;
