...
Node* addPoly(Node* x, Node* y) {
    ...
    Node* xpoly = x->next;   //x의 첫 번째 항에 대한 포인터(헤더 건너뛰기)
    Node* ypoly = y->next;   //y의 첫 번째 항에 대한 포인터(헤더 건너뛰기)
    Node* rst = result;  //결과의 마지막 항에 대한 포인터(초기 결과)

    while (xpoly && ypoly) {
        // printf(">>");printNode(rst);
         if (xpoly->exp > ypoly->exp) {// x에서 더 높은 지수를 갖는 항을 추가합니다.
                appendTerm(&rst, xpoly->coef, xpoly->exp);
                xpoly = xpoly->next;
            } 
            else if (xpoly->exp < ypoly->exp) {//더 낮은 지수를 사용하여 y에 항 추가
                appendTerm(&rst, ypoly->coef, ypoly->exp);
                ypoly = ypoly->next;
            } 
            else {
                //항의 지수가 동일한 경우, 계수의 합을 가지는 항 추가
                if (xpoly->coef + ypoly->coef != 0) {
                    appendTerm(&rst, ((xpoly->coef) + (ypoly->coef)), xpoly->exp );}
                    xpoly = xpoly->next;
                    ypoly = ypoly->next;
            }
        rst = rst ->next;  // Move k to the last term for the next iteration
    }
    // while(xpoly){appendTerm(&rst, xpoly->coef, xpoly->exp);xpoly = xpoly->next;rst = rst ->next; }
    // while(ypoly){appendTerm(&rst, ypoly->coef, ypoly->exp);ypoly = ypoly->next;rst = rst ->next; }
