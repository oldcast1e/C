...
int upRotate(IntStack *stack, int n);
int downRotate(IntStack *stack, int n);
/*스택 종료*/
...
int main() {
    int n,rep,num; scanf("%d",&n);
    
    IntStack *stack =  malloc(sizeof(int)*n);
    stack->max = 1000;

    char check[7][10] = {"POP", "PUSH" , "PEEK", "DUP", "UpR", "DownR", "PRINT"};
    char tmp[10],word,word_tmp;
    ...
    for(int i=0;i<rep;i++){
        printf("연산종류를 입력하세요: ");scanf("%s%c",tmp,&word_tmp); 
        for(int j=0;j<7;j++){//0부터 6까지 반복하여
        //입력받은 문자와 연산이름과 비교 후 일치하면 해당 연산 수행
            if(strcmp(tmp,check[j])==0){
                if(j==0) Pop(stack);
                else if(j==1){
                ...
                else if(j==2) {Peek(stack);break;}
                else if(j==3) {duplicate(stack);break;}
                else if(j==4){
                    // printf("Upr!!");
                    scanf("%d",&num);getchar();
                    upRotate(stack,n);
...
/*스택 초기화*/
int Initialisze(IntStack *stack,int max){
    stack->num = 0;
    ...
    */
    ...
    //동적할당 실패 시
    if((stack->stk)==NULL){stack->max = 0;return -1;}
...
void Print(IntStack *stack){
    for(int i=0;i<stack->num;i++) printf("%d ",stack->stk[i]);
...
int upRotate(IntStack *stack, int n){
    ...
    //단, n이 데이터의 개수보다 큰 경우에는 아무 작업을 하지 않는다.
    if( n > Size(stack)) return 0;
    for(int k=0;k<n;k++){
        char last = stack->stk[stack->num - 1];
        for(int i=stack->num-1;i>=0;i--){
            stack->stk[i+1] = stack->stk[i];
        }
        stack->stk[0] = last;
    }
    //스택의 데이터 수 만큼 반복하여 스택의 데이터를 출력
