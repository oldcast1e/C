...
int EnQue_rear(IntQueue *Que,int data);
int DeQue_front(IntQueue *Que);
int DeQue_rear(IntQueue *Que);
...
int main() {
    ...
    while(1){
        ...
        switch(menu){
            ...
            case 2:
                if(DeQue_front(&que) == -1) puts("디큐에 실패하였습니다.");
            ...
            case 10:
                ...
                if(Search(&que,x) == -1) printf("검색 실패\n");
                else{
            ...
            case 12:
                if(DeQue_rear(&que) == -1) puts("리어 디큐에 실패하였습니다.");
                else printf("디큐 데이터는 %d입니다.\n",x);
                break;
...
int DeQue_front(IntQueue *Que){//Basic
