...
int main() {
    ...
    while(1){
        ...
        printf("(1)푸시 (2)팝 (3)피크 (4)출력 (5)초기화 (6)용량확인 (7)큐의 현재 데이터 상태 (8)큐 저장상태 (9)큐 과저장 유무 (10)검색 (0)종료");
        ...
        switch(menu){
            case 1:
                ...
                if(EnQue(&que,x)==-1)puts("인큐에 실패하였습니다.");
                break;

            case 2:
                if(DeQue(&que,&x) == -1) puts("디큐에 실패하였습니다.");
