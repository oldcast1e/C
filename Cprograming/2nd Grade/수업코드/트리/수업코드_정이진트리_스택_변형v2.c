...
// 스택을 사용한 전위 순회 함수
char* InverseIterativePreOrder(TreeNode* root) {
    StackType S;
    Initialize(&S);

    char *rst = (char *)malloc(sizeof(char)*(N)); int cnt = 0;
    // StackType rst;
    // Initialize(&rst);
    if (root != NULL) Push(&S, root);
...
int main() {
    ...
    // TreeNode rst;
    // Initialize(&rst);
    char* rst =   InverseIterativePreOrder(n1); 
    printf("Post (InverseIterativePreOrder): "); for(int i=0;i<N;i++)printf("%c ",rst[i]);
    printf("\n");
    return 0;
}
