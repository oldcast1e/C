...
struct node *reduceExternal(struct node *z) {
    ...
    if (p->parent == NULL) {
        ...
        else if (p == g->rChild) g->rChild = zs;
    }
...
struct node* treeSearch(struct node *w, int k) {
    ...
    if (w->key == k) return w;
    if (w->key > k) return treeSearch(w->lChild, k);
...
int isBalanced(struct node *w) {
    if (-1 <= (w->lChild->height - w->rChild->height) && (w->lChild->height - w->rChild->height) <= 1) return 1;
    return 0;
}
