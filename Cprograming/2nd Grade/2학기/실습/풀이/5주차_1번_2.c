...
- rmergeSort(L) 함수: 단일연결리스트 L의 원소들을 합병 정렬하여 정렬된 결과를오름차순으로 정렬
- rmerge(L1, L2) 함수: 
    rmergeSort에 호출되어 두 개의 정렬된 단일연결리스트 L1과 L2를 합병한 하나의 단일연결리스트를 반환. 
...
tmp partition(listNode* L, int k) {
    //쉽게 말해, 그냥 |L|크기 (입력받은 노드)의 노드와 |L| – k 크기의 노드 두 개로 나누는 함수
    listNode* left;
    listNode* right;
...
listNode *rmerge(listNode * L1,listNode * L2){
    listNode *node =NULL;
    ...
    if (L1->data < L2->data) {
		...
		node = L1;
		node->next = rmerge(L1->next, L2);
	}
	else {
		node = L2;
		node->next = rmerge(L1, L2->next);
	}
...
function rmergeSort(L, size):
    if L is not NULL and size > 1:
        ...
        rmergeSort(L1, size // 2)  # Recursively sort first half
        rmergeSort(L2, ceil(size / 2))  # Recursively sort second half
        
        L = rmerge(L1, L2)  # Merge the sorted halves
*/
void rmergeSort(listType* L, int size){
    /* 초기화 필요 */
    ...
    if(orgin != NULL && size > 1){
        split = partition(orgin, size / 2); // =>> 노드를 두개로 나누어 split에 저장

        L1->head=split.L1; // 분할한 첫번째 노드를 저장
		L2->head=split.L2; // 분할한 두번째 노드를 저장

        rmergeSort(L1, size / 2); //재귀적 호출
        rmergeSort(L2, (int)(size / 2.0+0.5));//재귀적 호출
        
        L->head = rmerge(L1->head, L2->head);
    }
}
