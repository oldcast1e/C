...
element pop(IntStack *stack){
    ...
    element del = stack->stk[stack->num--];
    return del;
}
...
void upRotate(IntStack *stack, int n){
    if( n <= stack->num) {
        int save = stack->stk[stack->num-1]; // 맨 마지막 값
        for(int i=0;i<(n-1);i++){
...
void downRotate(IntStack *stack, int n){
    //밑으로 내리기
    if( n <= stack->num) {
        int save = stack->stk[0]; // 맨 첫 번째 값
        for(int i=0;i<(n-1);i++){
            printf("[%c] <- [%c]\n",stack->stk[stack->num - n + 1],stack->stk[stack->num - n]);
            stack->stk[stack->num - n + 1] = stack->stk[stack->num - n];//앞쪽으로 땡기기
        }
        stack->stk[n-1] = save;
    }
}
