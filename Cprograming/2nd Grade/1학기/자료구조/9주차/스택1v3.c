...
*/
void push(IntStack *stack, int stk){
    if(IsFull(stack)) printf("Stack FULL\n");
    else {
        // printf("stk : %c\n",stk);
        stack->stk[stack->num] = stk;
        stack->num ++;
    }
}
/*
...
element peek(IntStack *stack){
    element e = stack->stk[stack->num - 1];
    return e;
}
/*
...
void downRotate(IntStack* stack, int n) {
    if (n > stack->num + 1)
        return;
    char save = stack->stk[stack->num-n+1];
    for (int i = stack->num-n+1; i <=stack->num-1; i++) {
        stack->stk[i] = stack->stk[i + 1];
    }
    stack->stk[stack->num] = save;
}
/*
