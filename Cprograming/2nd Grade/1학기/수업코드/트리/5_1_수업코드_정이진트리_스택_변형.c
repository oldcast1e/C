...
// 스택을 사용한 전위 순회 함수
void InversePreOrder(TreeNode* root) {
    StackType S;
    Initialize(&S);
    if (root != NULL) Push(&S, root);
...
int main() {
    ...
    printf("Pre (Iterative): "); PreOrder(n1); printf("\n");
    printf("In (Iterative): "); InOrder(n1); printf("\n");
    printf("Post (Iterative): "); PostOrder(n1); printf("\n\n");
    printf("Post (InversePreOrder): ");InversePreOrder(n1); printf("\n");
    return 0;
}
