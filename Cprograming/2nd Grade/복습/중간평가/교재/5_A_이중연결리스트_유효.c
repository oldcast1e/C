...
void insertFirst(DListType* linkNode, element val){
    ...
    DListNode* H = linkNode->Head;/*노드 추가 시 기본 알고리즘*/
...
void insertLast(DListType* linkNode, element val){
    DListNode* new = (DListNode*)malloc(sizeof(DListNode));//새로운 노드 동적할당
    DListNode* Last = linkNode->Tail;/*노드 추가 시 기본 알고리즘*/
...
int deleteFirst(DListType* linkNode){//노드를 수정하려면 인자를 참조한다!!!
    DListNode* delete = linkNode->Head;
    if(delete == NULL){
        printf("Invailed Connection.\n");
        return -1;
    }
...
int deleteLast(DListType* linkNode){//노드를 수정하려면 인자를 참조한다!!!
    DListNode* delete = linkNode->Tail;
    DListNode* p = linkNode->Head;
