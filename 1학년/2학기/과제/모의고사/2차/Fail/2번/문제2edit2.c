...
void abs_val(struct complex *p1, struct complex *p2, struct complex *p3){
    ...
    //각 구조체 변수에 절대값 저장
    double max,min;
    if(p1->val_ab > p2->val_ab) max = p1->val_ab;
    else{max = p2->val_ab;}
    if(p1->val_ab > p3->val_ab) max = p1->val_ab;
    else{max = p3->val_ab;}
    if(p2->val_ab > p3->val_ab) max = p2->val_ab;
    else{max = p3->val_ab;}

    if(p1->val_ab < p2->val_ab) min = p1->val_ab;
    else{min = p2->val_ab;}
    if(p1->val_ab < p3->val_ab) min = p1->val_ab;
    else{min = p3->val_ab;}
    if(p2->val_ab < p3->val_ab) min = p2->val_ab;
    else{min = p3->val_ab;}
    printf("<%lf %lf>\n",max,min);
    p1->val_ab = max;
